...
import cn.hk.orange.tm.TransactionManagerImpl;
import cn.hk.orange.utils.Panic;
import cn.hk.orange.utils.ParseStringRes;
...
public class Table {
    ...
    public int update(long xid, Update update) throws Exception {
        ...
        if (fd == null) {
            throw ServerError.FieldNotFoundException;
        }
    ...
    private Map<String, Object> string2Entry(String[] values) throws Exception {
        if (values.length != fields.size()) {
            throw ServerError.InvalidValuesException;
        }
        ...
        if (where == null) {
            ...
            for (Field field : fields) {
                if (field.fieldName.equals(where.singleExp1.field)) {
                    if (!field.isIndexed()) {
                        throw ServerError.FieldNotIndexedException;
                    }
                    fd = field;
                    break;
                }
    ...
    private CalWhereRes calWhere(Field fd, Where where) throws Exception {
        ...
        switch (where.logicOp) {
            ...
            case "and":
                ...
                break;
            default:
                throw ServerError.InvalidLogOpException;
        }
        return res;
    }
